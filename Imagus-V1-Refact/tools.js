...
const T = (() => {
  ...
  function dot(ctx, x, y, sz, col) {
    ...
    if (customBrush && cur === 'brush') {
      ...
      const temp = document.createElement('canvas');
      temp.width = customBrush.width; temp.height = customBrush.height;
      ...
      for (let yy = 0; yy < temp.height; yy++)
        for (let xx = 0; xx < temp.width; xx++)
          if (!selHit(bx + xx, by + yy)) d[(yy * temp.width + xx) * 4 + 3] = 0;
  ...
  function floodFill(ctx, sx, sy, fh) {
    ...
    const t = tol, v = new Uint8Array(w * h);
    const st = [[sx, sy]];
    let iterations = 0;
