  ...
  function onUp(e) {
    ...
    if (cur === 'freehand-select' && sel && sel.type === 'freehand') {
      ...
      if (pts.length >= 3) {
        ...
        var sx = Math.max(0, Math.floor(x0)), sy = Math.max(0, Math.floor(y0));
        var sw = Math.min(C.W, Math.ceil(x1)) - sx, sh = Math.min(C.H, Math.ceil(y1)) - sy;
        if (sw > 0 && sh > 0) {
          ...
          for (var pi = 1; pi < pts.length; pi++) tx2.lineTo(pts[pi].x - sx, pts[pi].y - sy);
          tx2.closePath();
          tx2.fill();
          var mid = tx2.getImageData(0, 0, sw, sh);
          var mask = new Uint8Array(sw * sh);
          for (var mi = 0; mi < sw * sh; mi++) mask[mi] = mid.data[mi * 4 + 3] > 0 ? 1 : 0;
