...
const C = (() => {
  ...
  let grid = false, gsz = 8, gszH = 8, bg = 'transparent';
  let gridColor = '#4488ff', gridAlpha = 0.25, gridThick = 1;
  let pxGrid = false, pxGridColor = '#888899', pxGridAlpha = 0.2, pxGridThick = 1;
  let isoGrid = false, isoTileW = 64, isoTileH = 32;
  let isoColor = '#ff8800', isoAlpha = 0.35, isoThick = 1, isoShowDots = true, isoDotSize = 3;
  ...
  function mkCk() {
    ...
    _cachedGuideColor = getComputedStyle(document.documentElement).getPropertyValue('--guide').trim() || '#4488ff';
  }
  ...
  function render() {
    ...
    /* Isometric Grid — clipped to canvas, anchored at origin */
    if (isoGrid && grid) {
      const tw = Math.max(4, isoTileW);
      const th = Math.max(2, isoTileH);
      const halfW = tw / 2;
      const halfH = th / 2;
      if (zm * th >= 3) {
        ox.save();
        /* Clip so nothing draws outside the canvas area */
        ox.beginPath();
        ox.rect(Math.floor(ix(0)), Math.floor(iy(0)),
                Math.ceil(W * zm) + 1, Math.ceil(H * zm) + 1);
        ox.clip();

        ox.strokeStyle = isoColor;
        ox.globalAlpha = isoAlpha;
        ox.lineWidth = Math.max(1, isoThick);

        /* slope = rise/run of each diagonal line */
        const slope = th / tw;

        /*
         * Family A (↘ lines): y = slope·x + n·th
         * Family B (↙ lines): y = -slope·x + n·th
         *
         * n=0 gives lines through origin (0,0).
         * n ranges over all integers that produce visible lines.
         */
        const nMinA = Math.floor(-slope * W / th) - 1;
        const nMaxA = Math.ceil(H / th) + 1;
        for (let n = nMinA; n <= nMaxA; n++) {
          const c = n * th;
          ox.beginPath();
          ox.moveTo(Math.round(ix(0)), Math.round(iy(c)));
          ox.lineTo(Math.round(ix(W)), Math.round(iy(slope * W + c)));
          ox.stroke();
        }

        const nMinB = Math.floor(0 / th) - 1;
        const nMaxB = Math.ceil((H + slope * W) / th) + 1;
        for (let n = nMinB; n <= nMaxB; n++) {
          const c = n * th;
          ox.beginPath();
          ox.moveTo(Math.round(ix(0)), Math.round(iy(c)));
          ox.lineTo(Math.round(ix(W)), Math.round(iy(-slope * W + c)));
        ...
        /* Dots at every vertex where two diagonals cross.
         *
         * Vertices are at (i·halfW, j·halfH) where (i+j) is EVEN.
         * Row j=0: x = 0, tw, 2tw, ...          (i even)
         * Row j=1: x = halfW, halfW+tw, ...     (i odd)
         * Row j=2: x = 0, tw, 2tw, ...          (i even again)
         * This creates the perfect diamond lattice.
         */
        if (isoShowDots) {
          const dotR = Math.max(1, isoDotSize * 0.5 * Math.min(2, Math.max(0.5, zm * 0.3)));
          ox.fillStyle = isoColor;
          ox.globalAlpha = Math.min(1, isoAlpha + 0.3);
          for (let cy = 0; cy <= H; cy += halfH) {
            const row = Math.round(cy / halfH);
            const xOff = (row % 2 === 0) ? 0 : halfW;
            for (let cx = xOff; cx <= W; cx += tw) {
              ox.beginPath();
              ox.arc(Math.round(ix(cx)), Math.round(iy(cy)), dotR, 0, 6.2832);
              ox.fill();
            }
          }
        }
    ...
    /* Pixel Grid (1px per cell) */
    if (pxGrid && zm >= 4) {
      ox.save();
      ox.strokeStyle = pxGridColor;
      ox.globalAlpha = pxGridAlpha;
      ox.lineWidth = Math.max(0.5, pxGridThick * 0.5);
      for (let xg = 0; xg <= W; xg++) {
        const sx = snap(ix(xg));
        ox.beginPath(); ox.moveTo(sx, Math.round(iy(0)));
        ox.lineTo(sx, Math.round(iy(H))); ox.stroke();
      }
      for (let yg = 0; yg <= H; yg++) {
        const sy = snap(iy(yg));
        ox.beginPath(); ox.moveTo(Math.round(ix(0)), sy);
        ox.lineTo(Math.round(ix(W)), sy); ox.stroke();
      }
    ...
    /* Leak Highlight overlay */
    if (_leakHighlight) {
      const la = T.activeLayer();
      if (la) {
        const lw = W, lh = H, thr = 0;
        const src = la.ctx.getImageData(0, 0, lw, lh).data;
        /* Build binary: 1 = has content, 0 = empty */
        const solid = new Uint8Array(lw * lh);
        for (let i = 0; i < lw * lh; i++) {
          if (src[i * 4 + 3] > thr) solid[i] = 1;
        }
        /* Flood from all four edges */
        const leak = new Uint8Array(lw * lh);
        const q = [];
        for (let x = 0; x < lw; x++) {
          if (!solid[x]) { leak[x] = 1; q.push(x); }
          const bi = (lh - 1) * lw + x;
          if (!solid[bi]) { leak[bi] = 1; q.push(bi); }
        }
        for (let y = 1; y < lh - 1; y++) {
          const li = y * lw;
          if (!solid[li]) { leak[li] = 1; q.push(li); }
          const ri = y * lw + lw - 1;
          if (!solid[ri]) { leak[ri] = 1; q.push(ri); }
        }
        let qi = 0;
        while (qi < q.length) {
          const ci2 = q[qi++];
          const cx2 = ci2 % lw, cy2 = (ci2 - cx2) / lw;
          const nb = [ci2 - 1, ci2 + 1, ci2 - lw, ci2 + lw];
          const nx = [cx2 - 1, cx2 + 1, cx2, cx2];
          for (let n = 0; n < 4; n++) {
            const ni = nb[n];
            if (ni < 0 || ni >= lw * lh) continue;
            if (n < 2 && (nx[n] < 0 || nx[n] >= lw)) continue;
            if (!leak[ni] && !solid[ni]) { leak[ni] = 1; q.push(ni); }
          }
        }
        /* Draw red overlay on leak pixels that are empty */
        const ov = mx.createImageData(lw, lh);
        const od = ov.data;
        for (let i = 0; i < lw * lh; i++) {
          if (leak[i] && !solid[i]) {
            od[i * 4] = 255;
        ...
        ovCv.width = lw; ovCv.height = lh;
        ovCv.getContext('2d').putImageData(ov, 0, 0);
        mx.save();
        mx.globalAlpha = 1;
        mx.drawImage(ovCv, 0, 0);
        mx.restore();
      }
    }
    ...
    /* Move tool: show layer content bounds */
    if (T.showMoveBounds && T.current === 'move') {
      const bla = T.activeLayer();
      if (bla) {
        const bd = bla.ctx.getImageData(0, 0, W, H).data;
        ...
        if (bx1 >= 0) {
          ox.save();
          ox.strokeStyle = _cachedGuideColor;
          ox.lineWidth = 1;
          ox.setLineDash([4, 4]);
          ox.globalAlpha = 0.8;
          const rx = Math.round(ix(bx0));
          const ry = Math.round(iy(by0));
          const rw = Math.round(ix(bx1 + 1)) - rx;
          const rh = Math.round(iy(by1 + 1)) - ry;
          ox.strokeRect(rx + 0.5, ry + 0.5, rw, rh);
          ox.restore();
          ox.setLineDash([]);
        }
      }
    ...
    if (_curOn && _curX >= 0 && _curY >= 0 && _curX < W && _curY < H) {
      const bct = T.current;
      if (['brush','eraser','airbrush','smooth','lightdark','jumble'].includes(bct)) {
        const bcsz = bct === 'lightdark' ? T.ldSize : bct === 'smooth' ? Math.min(T.brushSize,16) : T.brushSize;
        const bch = bcsz / 2;
        ox.save();
        ox.translate(lf, tp);
        ox.scale(zm, zm);
        ...
        ox.lineWidth = 1.5 / zm;
        if (T.brushShape === 'square' && bct === 'brush') {
          ox.strokeRect(Math.floor(_curX - bch), Math.floor(_curY - bch), bcsz, bcsz);
        } else {
          ox.beginPath(); ox.arc(_curX, _curY, bch, 0, 6.2832); ox.stroke();
        }
        ox.strokeStyle = 'rgba(0,0,0,0.5)';
        ox.lineWidth = 0.75 / zm;
        if (T.brushShape === 'square' && bct === 'brush') {
          ox.strokeRect(Math.floor(_curX - bch), Math.floor(_curY - bch), bcsz, bcsz);
        } else {
          ox.beginPath(); ox.arc(_curX, _curY, bch, 0, 6.2832); ox.stroke();
    ...
    if (typeof Prev !== 'undefined' && Prev.update) Prev.update();
  }
  ...
  function togPxGrid() { pxGrid = !pxGrid; render(); }
  ...
  return {
    ...
    get gszH() { return gszH; }, set gszH(v) { gszH = v; },
    get bg() { return bg; },
    ...
    togGrid, togPxGrid, flipC, rotC, resize, compClean, compFrame, mkCk,
    get pxGrid() { return pxGrid; }, set pxGrid(v) { pxGrid = v; render(); },
    get pxGridColor() { return pxGridColor; }, set pxGridColor(v) { pxGridColor = v; },
    get pxGridAlpha() { return pxGridAlpha; }, set pxGridAlpha(v) { pxGridAlpha = v; },
    get pxGridThick() { return pxGridThick; }, set pxGridThick(v) { pxGridThick = v; },
    get isoGrid() { return isoGrid; }, set isoGrid(v) { isoGrid = v; render(); },
    get isoTileW() { return isoTileW; }, set isoTileW(v) { isoTileW = v; },
    get isoTileH() { return isoTileH; }, set isoTileH(v) { isoTileH = v; },
    get isoColor() { return isoColor; }, set isoColor(v) { isoColor = v; },
    get isoAlpha() { return isoAlpha; }, set isoAlpha(v) { isoAlpha = v; },
    get isoThick() { return isoThick; }, set isoThick(v) { isoThick = v; },
    get isoShowDots() { return isoShowDots; }, set isoShowDots(v) { isoShowDots = v; },
    get isoDotSize() { return isoDotSize; }, set isoDotSize(v) { isoDotSize = v; }
  };
})();
...
const Ly = (() => {
  ...
  function _blLum(r, g, b) { return 0.3 * r + 0.59 * g + 0.11 * b; }
  function _blSat(r, g, b) { return Math.max(r, g, b) - Math.min(r, g, b); }
  function _blClip(r, g, b) {
    var l = _blLum(r, g, b), n = Math.min(r, g, b), x = Math.max(r, g, b);
    if (n < 0) { var d = l - n; r = l + (r - l) * l / d; g = l + (g - l) * l / d; b = l + (b - l) * l / d; }
    if (x > 255) { var d = x - l; r = l + (r - l) * (255 - l) / d; g = l + (g - l) * (255 - l) / d; b = l + (b - l) * (255 - l) / d; }
    return [_clamp255(r), _clamp255(g), _clamp255(b)];
  }
  function _blSetLum(r, g, b, l) { var d = l - _blLum(r, g, b); return _blClip(r + d, g + d, b + d); }
  function _blSetSat(r, g, b, s) {
    var a = [{v:r,i:0}, {v:g,i:1}, {v:b,i:2}];
    a.sort(function(x, y) { return x.v - y.v; });
    var o = [0, 0, 0];
    if (a[2].v > a[0].v) { o[a[1].i] = ((a[1].v - a[0].v) * s) / (a[2].v - a[0].v); o[a[2].i] = s; }
    return o;
  }
  ...
  function _vividLightCh(b, s) {
    return s <= 128
      ...
      : _colorDodgeCh(b, Math.min(255, 2 * s - 255));
  }
  function _linearLightCh(b, s) { return _clamp255(b + 2 * s - 255); }
  function _pinLightCh(b, s) {
    return s <= 128 ? Math.min(b, s * 2) : Math.max(b, 2 * s - 255);
  }
  ...
  function _blendRGB(mode, br, bg, bb, sr, sg, sb) {
    switch (mode) {
      ...
      case 'hue': {
        var hs = _blSetSat(sr, sg, sb, _blSat(br, bg, bb));
        return _blSetLum(hs[0], hs[1], hs[2], _blLum(br, bg, bb));
      }
      case 'saturation': {
        var ss2 = _blSetSat(br, bg, bb, _blSat(sr, sg, sb));
        return _blSetLum(ss2[0], ss2[1], ss2[2], _blLum(br, bg, bb));
      }
      case 'color': {
        return _blSetLum(sr, sg, sb, _blLum(br, bg, bb));
      }
      case 'luminosity': {
        return _blSetLum(br, bg, bb, _blLum(sr, sg, sb));
      }
      default:
        return [sr, sg, sb];
    }
  ...
  /**
   * Returns a layer name guaranteed unique within the given frame.
   * If "Layer 2" already exists, returns "Layer 2 #2", then "Layer 2 #3", etc.
   */
  function _uniqueName(baseName, frame) {
    const existing = new Set(frame.layers.map(ly => ly.name));
  ...
  function flattenVisible() {
    const f = fr(); if (!f) return;
    /* Count visible layers — need at least 2 to merge */
    const visCount = f.layers.filter(ly => ly.visible).length;
    if (visCount < 2) { alert('Need at least 2 visible layers to flatten.'); return; }
    Hi.push();
    /* Composite all visible layers into one */
    const merged = mk('Flat Visible');
    if (C.bg === 'white') {
      merged.ctx.fillStyle = '#fff';
      merged.ctx.fillRect(0, 0, C.W, C.H);
    }
    for (const ly of f.layers) {
      if (!ly.visible) continue;
      Ly.drawTo(merged.ctx, ly);
    }
    merged.ctx.globalAlpha = 1;
    /* Keep invisible layers, replace visible ones with the merged layer */
    const kept = f.layers.filter(ly => !ly.visible);
    /* Insert merged layer at the top */
    kept.push(merged);
    f.layers = kept;
    f.activeLayer = f.layers.length - 1;
    ui(); C.render();
  ...
  function syncAll() {
    ...
    for (let i = 0; i < An.frames.length; i++) {
      ...
      const cp = clone(src);
      cp.name = _uniqueName(src.name, frame);
  ...
  function renAll() {
    ...
    for (var fi = 0; fi < An.frames.length; fi++) {
      var fr2 = An.frames[fi];
      for (var li = 0; li < fr2.layers.length; li++) {
        var nm = fr2.layers[li].name;
        if (!seen[nm]) { seen[nm] = []; names.push(nm); }
        seen[nm].push(fi + 1);
      }
    }
    function compact(arr) {
      if (arr.length === An.frames.length) return 'all ' + arr.length + ' frames';
      var parts = [], i = 0;
      while (i < arr.length) {
        var s = arr[i], e = s;
    ...
    var h = '<h3>Rename Layer (All Frames)</h3>';
    h += '<label>Layer<select id="ra_src" style="width:160px">';
    for (var i = 0; i < names.length; i++) {
      h += '<option value="' + names[i].replace(/"/g, '&quot;') + '">' + names[i] + ' (' + compact(seen[names[i]]) + ')</option>';
    }
    h += '</select></label>';
    h += '<label>New Name<input type="text" id="ra_nm" value="" maxlength="60" style="width:140px"></label>';
    h += '<p style="font-size:8px;color:var(--txd);margin-top:4px">Renames this layer in every frame where it appears.</p>';
    h += '<div class="mb"><button class="bsc" onclick="U.cMo()">Cancel</button><button class="bp" id="ra_ok">Rename</button></div>';
    U.sMo(h);
    var srcEl = document.getElementById('ra_src');
    document.getElementById('ra_nm').value = srcEl.value;
    srcEl.onchange = function() { document.getElementById('ra_nm').value = this.value; };
    document.getElementById('ra_ok').onclick = function() {
      var oldName = document.getElementById('ra_src').value;
      var newName = (document.getElementById('ra_nm').value || '').trim();
      if (!newName) { alert('Name cannot be empty.'); return; }
      if (newName === oldName) { U.cMo(); return; }
      var conflict = [];
      for (var fi = 0; fi < An.frames.length; fi++) {
        var fr2 = An.frames[fi];
        for (var li = 0; li < fr2.layers.length; li++) {
          if (fr2.layers[li].name === newName) {
            var hasOld = false;
            for (var li2 = 0; li2 < fr2.layers.length; li2++) {
              if (fr2.layers[li2].name === oldName) { hasOld = true; break; }
            }
            if (hasOld) conflict.push(fi + 1);
            break;
          }
        }
      }
      if (conflict.length) { alert('Name "' + newName + '" already exists in frame(s): ' + conflict.join(', ')); return; }
      Hi.push();
      for (var fi = 0; fi < An.frames.length; fi++) {
        var fr2 = An.frames[fi];
        for (var li = 0; li < fr2.layers.length; li++) {
          if (fr2.layers[li].name === oldName) fr2.layers[li].name = newName;
        }
      }
      U.cMo(); ui(); C.render();
    };
  ...
  function ren() {
    ...
    if (!n || !n.trim()) return;
    const nm = n.trim();
    if (nm === ly.name) return;
    for (var i = 0; i < f.layers.length; i++) {
      if (i !== f.activeLayer && f.layers[i].name === nm) {
        alert('A layer named "' + nm + '" already exists in this frame (layer ' + (i + 1) + '). Choose a different name.');
        return;
      }
    }
    Hi.push(); ly.name = nm; ui();
  }

  function linkCel() {
    var f = fr(); if (!f) return;
    var ly = f.layers[f.activeLayer]; if (!ly) return;
    Hi.push();
    for (var fi = 0; fi < An.frames.length; fi++) {
      if (fi === An.cf) continue;
      var oFr = An.frames[fi];
      for (var li = 0; li < oFr.layers.length; li++) {
        if (oFr.layers[li].name === ly.name) {
          oFr.layers[li].canvas = ly.canvas;
          oFr.layers[li].ctx = ly.ctx;
        }
      }
    }
    ui(); C.render(); An.uStrip();
  }
  ...
  function unlinkCel() {
    ...
    for (var fi = 0; fi < An.frames.length; fi++) {
      ...
      var oFr = An.frames[fi];
      for (var li = 0; li < oFr.layers.length; li++) {
        if (oFr.layers[li].name === ly.name && oFr.layers[li].canvas === ly.canvas) {
          var nc = document.createElement('canvas');
          nc.width = C.W; nc.height = C.H;
          var nx = nc.getContext('2d', { willReadFrequently: true });
          nx.imageSmoothingEnabled = false;
          nx.drawImage(ly.canvas, 0, 0);
          oFr.layers[li].canvas = nc;
          oFr.layers[li].ctx = nx;
        }
      }
    }
    ui(); C.render(); An.uStrip();
  ...
  function sOp() {
    const f = fr(); if (!f) return;
    if (f.activeLayer < 0 || f.activeLayer >= f.layers.length) return;
    const ly = f.layers[f.activeLayer]; if (!ly) return;
    const lopEl = document.getElementById('lop');
    const lovEl = document.getElementById('lov');
    if (lopEl) lopEl.value = ly.opacity;
    if (lovEl) lovEl.textContent = ly.opacity;
  ...
  return {
    mk, clone, ensureMask, cloneMask, isMaskEditing, _findByName, _uniqueName, linkCel, unlinkCel,
    addMask, cycleMask, drawTo,
    paintMaskDot, paintMaskLine, applyMask, syncMaskToAllFrames,
    setBlendMode, openBlendDialog,
    add, rm, dup, up, dn, mergeDown, flatten, flattenVisible,
    syncAll, syncSel, rmAll, detachAll, detachSel, ren, renAll, ui, sOp
  };
})();
...
const An = (() => {
  ...
  let tags = [];
  let playMode = 'forward', _ppDir = 1;

  function mkF() {
    return { layers: [Ly.mk('Layer 1')], activeLayer: 0, duration: 100 };
  }

  function init() { frames = [mkF()]; cf = 0; tags = []; uStrip(); Ly.ui(); }
  ...
  function tick() {
    if (!playing) return;
    if (playMode === 'ping-pong') {
      const next = cf + _ppDir;
      if (next >= frames.length) { _ppDir = -1; cf = frames.length - 2; }
      else if (next < 0) { _ppDir = 1; cf = 1; }
      else { cf = next; }
      cf = Math.max(0, Math.min(frames.length - 1, cf));
    } else if (playMode === 'reverse') {
      cf = (cf - 1 + frames.length) % frames.length;
    } else {
  ...
  function stop() {
    ...
    _ppDir = 1;
    if (_ds) { _ds = false; uStrip(); Ly.ui(); C.render(); syncDur(); }
  }
  ...
  function uStrip() {
    ...
    frames.forEach((fr, i) => {
      ...
      const tag = getTagAt(i);
      if (tag) div.style.borderTopColor = tag.color;
      const c2 = document.createElement('canvas');
      ...
      const tl = tag ? tag.name.slice(0, 6) + ' ' : '';
      sp.textContent = tl + (i + 1) + ' ' + fr.duration + 'ms';
      div.append(c2, sp);
      div.onclick = e => {
        if (e.shiftKey) {
  ...
  /* Frame tags */
  function getTagAt(fi) {
    return tags.find(t => fi >= t.from && fi <= t.to) || null;
  }

  function addTag(name, from, to, color) {
    if (!name || from < 0 || to < from || to >= frames.length) return;
    tags.push({ name, from, to, color: color || '#44aaff' });
    uStrip();
  }

  function removeTag(idx) {
    if (idx >= 0 && idx < tags.length) { tags.splice(idx, 1); uStrip(); }
  }

  function setPlayMode(mode) {
    if (['forward', 'reverse', 'ping-pong'].includes(mode)) playMode = mode;
  }
  ...
  return {
    ...
    get tags() { return tags; }, set tags(v) { tags = v; },
    get playMode() { return playMode; }, set playMode(v) { setPlayMode(v); },
    mkF, init, addF, dupF, rmF, goTo, prev, next, moveL, moveR,
    selAllF, pp, stop, togOnion, togOnionAll, uStrip, syncDur, tween, moveBlockTo,
    addTag, removeTag, getTagAt, setPlayMode
  };
})();

/* ===== HISTORY — delta-based undo ===== */
const Hi = (() => {
  let u = [], r = [];

  /* Adaptive cap: limits undo depth based on memory usage */
  function maxDepth() {
    const pixelBytes = C.W * C.H * 4;
    /* Estimate bytes per snapshot: only the active frame's layers */
    const fr = An.frames[An.cf];
    const surfaces = fr ? fr.layers.reduce((n, ly) => n + 1 + (ly.mask ? 1 : 0), 0) : 1;
    const snapshotBytes = surfaces * pixelBytes;
    const MAX_BYTES = 200 * 1024 * 1024; /* 200 MB budget */
    return Math.max(8, Math.min(60, Math.floor(MAX_BYTES / Math.max(1, snapshotBytes))));
  }

  /* Snap only the current frame + metadata */
  function snap() {
    const ci = An.cf;
    const fr = An.frames[ci];
    if (!fr) return { cf: ci, w: C.W, h: C.H, mode: 'delta', fi: ci, frame: null };
    const fl = { al: fr.activeLayer, dur: fr.duration, layers: [] };
    for (const ly of fr.layers) {
      const c = document.createElement('canvas');
      c.width = C.W; c.height = C.H;
      ...
      fl.layers.push({
        ...
        k: ly.locked, b: ly.blendMode, c, m
      });
    }
    return { cf: ci, w: C.W, h: C.H, mode: 'delta', fi: ci, frame: fl };
  }

  /* Full snap — used only for operations that touch ALL frames
     (e.g., resize, rotate canvas, convert-to-palette, tween) */
  function snapFull() {
    const out = { cf: An.cf, w: C.W, h: C.H, mode: 'full', frames: [] };
  ...
  function _restoreFrame(fs, fi) {
    if (!fs || fi < 0 || fi >= An.frames.length) return;
  ...
  function restore(s) {
    ...
    if (s.mode === 'full') {
      ...
      /* Delta restore — single frame */
      if (sizeChanged) C.init(rw, rh, C.bg);
      const fi = s.fi;
      if (fi >= 0 && fi < An.frames.length && s.frame) {
        _restoreFrame(s.frame, fi);
      }
      An.cf = Math.min(s.cf, An.frames.length - 1);
    }

    if (sizeChanged) { C.zFit(); }
    else { C.zm = prevZoom; C.px = prevPanX; C.py = prevPanY; C.render(); }
    Ly.ui(); An.uStrip();
  }
  ...
  /* pushFull — call this instead of push() for operations
     that affect multiple frames (resize, rotate, tween, etc.) */
  function pushFull() {
    _dirty = true;
    const cap = Math.max(3, Math.floor(maxDepth() / 3));
    u.push(snapFull());
    while (u.length > cap) u.shift();
    r = [];
  }
  ...
  function undo() {
    ...
    r.push(entry.mode === 'full' ? snapFull() : snap());
  ...
  function redo() {
    ...
    u.push(entry.mode === 'full' ? snapFull() : snap());
    restore(r.pop());
  }

  function clear() { u = []; r = []; _dirty = false; }

  return { push, pushFull, undo, redo, clear };
})();
