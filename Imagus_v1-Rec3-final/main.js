...
const U = (() => {
  ...
  function uTopt() {
    ...
    else if (t === 'move') b.innerHTML = `<label>All Frames<input type="checkbox" ${T.moveAllFrames ? 'checked' : ''} onchange="T.moveAllFrames=this.checked" title="Move this layer in all frames that contain it"></label><label>Show Bounds<input type="checkbox" ${T.showMoveBounds ? 'checked' : ''} onchange="T.showMoveBounds=this.checked;C.render()" title="Show selection outline for layer content"></label>`;
        ...
        if (lines[0] && lines[0].trim() === 'JASC-PAL') {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (/^[0-9a-fA-F]{6}$/.test(h)) {
              ...
              colors.push('#' + h.slice(2, 8).toLowerCase());
            }
          }
        }
  ...
  function gridCfg() {
    sMo(`<h3>Grid Settings</h3>
<label style="font-weight:600;color:var(--acc)">▦ Rectangular Grid</label>
<label>Width<input type="number" id="gss" value="${C.gsz}" min="1" max="256" style="width:60px"></label>
<label>Height<input type="number" id="gsh" value="${C.gszH}" min="1" max="256" style="width:60px"></label>
<label>Lock W/H<input type="checkbox" id="gsl" checked></label>
<label>Color<input type="color" id="gcol" value="${C.gridColor}" style="width:40px;height:22px;padding:0;border:none"></label>
<label>Opacity %<input type="number" id="gopa" value="${Math.round(C.gridAlpha * 100)}" min="5" max="100" style="width:50px"></label>
<label>Thickness<input type="number" id="gthk" value="${C.gridThick}" min="1" max="5" style="width:50px"></label>
<div class="sep"></div>
<label style="font-weight:600;color:var(--acc)">⊹ Guides</label>
<label>Guide H<input type="checkbox" id="ggh" ${C.guideH ? 'checked' : ''}></label>
<label>Guide V<input type="checkbox" id="ggv" ${C.guideV ? 'checked' : ''}></label>
<label>GH pos %<input type="number" id="ghy" value="${Math.round(C.guideHY * 100)}" min="0" max="100" style="width:50px"></label>
<label>GV pos %<input type="number" id="gvx" value="${Math.round(C.guideVX * 100)}" min="0" max="100" style="width:50px"></label>
<div class="sep"></div>
<label style="font-weight:600;color:var(--acc)">⊞ Pixel Grid</label>
<label>Color<input type="color" id="pgcol" value="${C.pxGridColor}" style="width:40px;height:22px;padding:0;border:none"></label>
<label>Opacity %<input type="number" id="pgopa" value="${Math.round(C.pxGridAlpha * 100)}" min="5" max="100" style="width:50px"></label>
<label>Thickness<input type="number" id="pgthk" value="${C.pxGridThick}" min="1" max="3" style="width:50px"></label>
<div class="sep"></div>
<label style="font-weight:600;color:#ff8800">◆ Isometric Grid</label>
<label>Enabled<input type="checkbox" id="ig_on" ${C.isoGrid ? 'checked' : ''}></label>
<label>Tile W<input type="number" id="ig_tw" value="${C.isoTileW}" min="4" max="512" style="width:60px"></label>
<label>Tile H<input type="number" id="ig_th" value="${C.isoTileH}" min="2" max="512" style="width:60px"></label>
<label>Ratio
  <select id="ig_ratio" style="width:100px">
    <option value="2:1">2:1 Standard</option>
    <option value="3:1">3:1 Steep</option>
    <option value="1:1">1:1 (45°)</option>
    <option value="custom">Custom</option>
  </select>
</label>
<label>Color<input type="color" id="ig_col" value="${C.isoColor}" style="width:40px;height:22px;padding:0;border:none"></label>
<label>Opacity %<input type="number" id="ig_opa" value="${Math.round(C.isoAlpha * 100)}" min="5" max="100" style="width:50px"></label>
<label>Thickness<input type="number" id="ig_thk" value="${C.isoThick}" min="1" max="5" style="width:50px"></label>
<label>Show Dots<input type="checkbox" id="ig_dots" ${C.isoShowDots ? 'checked' : ''}></label>
<label>Dot Size<input type="number" id="ig_dsz" value="${C.isoDotSize}" min="1" max="10" style="width:50px"></label>
<div class="mb"><button class="bsc" onclick="U.cMo()">Cancel</button><button class="bp" onclick="U.applyGrid()">OK</button></div>`);
    /* Lock W/H */
    document.getElementById('gss').oninput = function() {
      if (document.getElementById('gsl').checked)
        document.getElementById('gsh').value = this.value;
    };
    document.getElementById('gsh').oninput = function() {
      if (document.getElementById('gsl').checked)
        document.getElementById('gss').value = this.value;
    };
    /* Isometric ratio wiring */
    const igR = document.getElementById('ig_ratio');
    const igW = document.getElementById('ig_tw');
    const igH = document.getElementById('ig_th');
    if (igR && igW && igH) {
      const cw = +igW.value, ch = +igH.value;
      if (ch === Math.round(cw / 2)) igR.value = '2:1';
      else if (ch === Math.round(cw / 3)) igR.value = '3:1';
      else if (ch === cw) igR.value = '1:1';
      else igR.value = 'custom';
      igH.disabled = igR.value !== 'custom';
      function _isoSync() {
        const w = Math.max(4, +igW.value || 64);
        if (igR.value === '2:1') igH.value = Math.max(2, Math.round(w / 2));
        else if (igR.value === '3:1') igH.value = Math.max(2, Math.round(w / 3));
        else if (igR.value === '1:1') igH.value = w;
      }
      igR.onchange = function() { _isoSync(); igH.disabled = this.value !== 'custom'; };
      igW.oninput = _isoSync;
    }
  }

  function applyGrid() {
    C.gsz = Math.max(1, +document.getElementById('gss').value || 8);
    C.gszH = Math.max(1, +document.getElementById('gsh').value || 8);
    C.gridColor = document.getElementById('gcol').value;
    C.gridAlpha = Math.max(0.05, Math.min(1, (+document.getElementById('gopa').value) / 100));
    C.gridThick = Math.max(1, Math.min(5, +document.getElementById('gthk').value || 1));
    C.guideH = document.getElementById('ggh').checked;
    C.guideV = document.getElementById('ggv').checked;
    C.guideHY = Math.max(0, Math.min(1, (+document.getElementById('ghy').value) / 100));
    C.guideVX = Math.max(0, Math.min(1, (+document.getElementById('gvx').value) / 100));
    const pgcEl = document.getElementById('pgcol');
    if (pgcEl) {
      C.pxGridColor = pgcEl.value;
      C.pxGridAlpha = Math.max(0.05, Math.min(1, (+document.getElementById('pgopa').value) / 100));
      C.pxGridThick = Math.max(1, Math.min(3, +document.getElementById('pgthk').value || 1));
    }
    const igOn = document.getElementById('ig_on');
    if (igOn) {
      C.isoGrid = igOn.checked;
      C.isoTileW = Math.max(4, +document.getElementById('ig_tw').value || 64);
      C.isoTileH = Math.max(2, +document.getElementById('ig_th').value || 32);
      C.isoColor = document.getElementById('ig_col').value;
      C.isoAlpha = Math.max(0.05, Math.min(1, (+document.getElementById('ig_opa').value) / 100));
      C.isoThick = Math.max(1, Math.min(5, +document.getElementById('ig_thk').value || 1));
      C.isoShowDots = document.getElementById('ig_dots').checked;
      C.isoDotSize = Math.max(1, Math.min(10, +document.getElementById('ig_dsz').value || 3));
    }
    C.render(); cMo();
  }
  ...
  function checkerCfg() {
    ...
    const thm1 = cs.getPropertyValue('--ck1').trim() || '#bbbbcc';
    const thm2 = cs.getPropertyValue('--ck2').trim() || '#ccccdd';
    sMo(`<h3>Checker</h3>
<label>Width<input type="number" id="ckw" value="${C.ckSize}" min="1" max="64" style="width:50px"></label>
<label>Height<input type="number" id="ckh" value="${C.ckSizeH || C.ckSize}" min="1" max="64" style="width:50px"></label>
<label>Lock<input type="checkbox" id="cklock" checked></label>
<label>Color 1<input type="color" id="ckc1" value="${C.ckCol1 || thm1}" style="width:40px;height:20px;padding:0"></label>
<label>Color 2<input type="color" id="ckc2" value="${C.ckCol2 || thm2}" style="width:40px;height:20px;padding:0"></label>
<label><button class="bsc" style="font-size:8px;padding:1px 4px" id="ck_reset">Reset to Theme</button></label>
<div class="mb"><button class="bsc" onclick="U.cMo()">Cancel</button><button class="bp" id="ck_ok">OK</button></div>`);
    document.getElementById('ckw').oninput = function() {
      if (document.getElementById('cklock').checked)
        document.getElementById('ckh').value = this.value;
    };
    document.getElementById('ckh').oninput = function() {
      if (document.getElementById('cklock').checked)
        document.getElementById('ckw').value = this.value;
    };
    document.getElementById('ck_reset').onclick = function() {
      document.getElementById('ckc1').value = thm1;
      document.getElementById('ckc2').value = thm2;
    };
    document.getElementById('ck_ok').onclick = function() {
      C.ckCol1 = document.getElementById('ckc1').value;
      C.ckCol2 = document.getElementById('ckc2').value;
      C.ckSize = Math.max(1, +document.getElementById('ckw').value || 8);
      U.cMo();
    };
  }
...
<div class="mb"><button class="bsc" onclick="U._adjCancel()">Cancel</button><button class="bsc" onclick="U.prevFx()">Preview</button><button class="bp" onclick="U.apEffect()">Apply</button></div>`);
  ...
  function togLeak() {
    _leakHighlight = !_leakHighlight;
    const el = document.getElementById('mi_leak');
    if (el) el.textContent = (_leakHighlight ? '\u2713 ' : '') + 'Leak Highlight';
    C.render();
  }
...
var _cpBk = null, _cpTimer = 0;

  function convertToPal() {
    if (!userPalette.length) { alert('Load a palette first.'); return; }
    _cpBk = [];
    for (var f = 0; f < An.frames.length; f++) {
      var fb = [];
      for (var l = 0; l < An.frames[f].layers.length; l++) {
        fb.push(An.frames[f].layers[l].ctx.getImageData(0, 0, C.W, C.H));
      }
      _cpBk.push(fb);
    }
    var ev = 'oninput="this.nextElementSibling.textContent=this.value;U.prevConvertToPal()"';
    var ev2 = 'onchange="U.prevConvertToPal()"';
    var h = '<h3>Convert to Palette</h3>';
    h += '<label>Dithering<select id="cp_style" style="width:120px" ' + ev2 + '>';
    var styles = ['None','Floyd-Steinberg','False Floyd','Atkinson','Sierra 2','Sierra Lite','Stucki','Two-D','Right','Down','Double Down','Diagonal','Diamond','Ordered','Blue Noise'];
    for (var i = 0; i < styles.length; i++) h += '<option' + (i===1?' selected':'') + '>' + styles[i] + '</option>';
    h += '</select></label>';
    h += '<label>Color Match<select id="cp_match" style="width:120px" ' + ev2 + '><option selected>Distance</option><option>Percentage</option><option>Hue-Based</option><option>Maximum</option></select></label>';
    h += '<label>Diversity<input type="range" id="cp_div" min="0" max="100" value="50" style="width:80px" ' + ev + '><span>50</span></label>';
    h += '<label>Diffusion<input type="range" id="cp_dif" min="0" max="100" value="100" style="width:80px" ' + ev + '><span>100</span></label>';
    h += '<label>Brightness<input type="range" id="cp_bri" min="-100" max="100" value="0" style="width:80px" ' + ev + '><span>0</span></label>';
    h += '<label>Contrast<input type="range" id="cp_con" min="-100" max="100" value="0" style="width:80px" ' + ev + '><span>0</span></label>';
    h += '<label>Color<input type="range" id="cp_sat" min="0" max="200" value="100" style="width:80px" ' + ev + '><span>100</span></label>';
    h += '<label>Golden Ratio<input type="checkbox" id="cp_gold" ' + ev2 + '></label>';
    h += '<p style="font-size:8px;color:var(--txd);margin-top:4px">Preview: current frame. Apply: ALL frames (' + userPalette.length + ' colors)</p>';
    h += '<div class="mb"><button class="bsc" onclick="U._cpCancel()">Cancel</button><button class="bp" onclick="U.apConvertToPal()">Apply</button></div>';
    sMo(h);
    prevConvertToPal();
  }
  ...
  function togColorMode() {
    if (_colorMode === 'rgb') {
      if (!confirm('Convert all layers to CMYK-safe colors?\nThis replaces out-of-gamut colors with the closest CMYK alternative.\nThis is destructive and cannot be reversed except with Ctrl+Z.')) return;
      Hi.pushFull();
      /* RGB → CMYK → RGB round-trip clamps to CMYK gamut */
      for (const fr of An.frames) {
        for (const ly of fr.layers) {
          const id = ly.ctx.getImageData(0, 0, C.W, C.H);
          const d = id.data;
          for (let i = 0; i < d.length; i += 4) {
            if (d[i + 3] === 0) continue;
            ...
            const cy = (1 - b - k) * ik;
            /* CMYK → RGB (clamped) */
            d[i]     = Math.round((1 - Math.min(1, cc)) * (1 - k) * 255);
            d[i + 1] = Math.round((1 - Math.min(1, cm)) * (1 - k) * 255);
            d[i + 2] = Math.round((1 - Math.min(1, cy)) * (1 - k) * 255);
          }
          ly.ctx.putImageData(id, 0, 0);
        }
      }
      _colorMode = 'cmyk';
      document.getElementById('mi_cmode').textContent = '✓ Color Mode: CMYK';
      C.render(); Ly.ui();
    } else {
      _colorMode = 'rgb';
      document.getElementById('mi_cmode').textContent = 'Color Mode: RGB';
    }
  }

  function _cpRunLayer(srcData, w, ht, palRGB, style, match, divF, difF, bri, con, sat, golden) {
    ...
    var kernels = {
      ...
      'Right':           [[1,0,1]],
      'Down':            [[0,1,1]],
      'Double Down':     [[0,1,0.5],[0,2,0.5]],
      'Diagonal':        [[1,1,1]],
      'Diamond':         [[1,0,3/8],[-1,1,1/8],[0,1,3/8],[1,1,1/8]],
      'Two-D':           [[1,0,0.5],[0,1,0.5]]
    };
    function near(r, g, b) {
      var best = 0, bestD = 1e9;
      for (var j = 0; j < palRGB.length; j++) {
        var pr = palRGB[j].r, pg = palRGB[j].g, pb = palRGB[j].b;
        var dist;
        if (match === 'Maximum') { dist = Math.max(Math.abs(r-pr), Math.abs(g-pg), Math.abs(b-pb)); }
        else if (match === 'Hue-Based') { var dr=r-pr, dg=g-pg, db=b-pb, rm=(r+pr)/2; dist = (2+rm/256)*dr*dr + 4*dg*dg + (2+(255-rm)/256)*db*db; }
        else if (match === 'Percentage') { dist = Math.pow((r-pr)/2.55, 2) + Math.pow((g-pg)/2.55, 2) + Math.pow((b-pb)/2.55, 2); }
        else { dist = (r-pr)*(r-pr) + (g-pg)*(g-pg) + (b-pb)*(b-pb); }
        if (dist < bestD) { bestD = dist; best = j; }
        if (dist === 0) break;
      }
      return palRGB[best];
    }
    var cf = (259 * (con + 255)) / (255 * (259 - con));
    if (bri !== 0 || con !== 0 || sat !== 100) {
      for (var i = 0; i < d.length; i += 4) {
        if (d[i+3] === 0) continue;
        var ar = d[i]+bri, ag = d[i+1]+bri, ab = d[i+2]+bri;
        ar = cf*(ar-128)+128; ag = cf*(ag-128)+128; ab = cf*(ab-128)+128;
        if (sat !== 100) { var gy = 0.299*ar+0.587*ag+0.114*ab, sf = sat/100; ar = gy+sf*(ar-gy); ag = gy+sf*(ag-gy); ab = gy+sf*(ab-gy); }
        d[i] = cl(ar); d[i+1] = cl(ag); d[i+2] = cl(ab);
      }
    }
    if (style === 'None') {
      for (var y = 0; y < ht; y++) {
        for (var x = 0; x < w; x++) {
          ...
          var nr = d[idx], ng = d[idx+1], nb = d[idx+2];
          if (golden) {
            var gn = (((x * phi + y * phi * phi) % 1) - 0.5) * divF * 48;
            nr = cl(nr + gn); ng = cl(ng + gn); nb = cl(nb + gn);
          }
          var c = near(nr, ng, nb);
          d[idx] = c.r; d[idx+1] = c.g; d[idx+2] = c.b;
        }
      }
    } else if (style === 'Ordered' || style === 'Blue Noise') {
      for (var y = 0; y < ht; y++) {
        for (var x = 0; x < w; x++) {
          var idx = (y * w + x) * 4;
          if (d[idx+3] === 0) continue;
          var t;
          if (style === 'Ordered') {
            t = (bayer[(y%4)*4+(x%4)] / 16 - 0.5) * divF * 64;
            if (golden) t *= (phi - 1);
          } else {
            if (golden) { t = (((x*phi%1)+(y*phi%1))/2-0.5) * divF * 64; }
            else { t = ((((x*13+y*7+x*y)%64)/64)-0.5) * divF * 64; }
          }
          var c = near(cl(d[idx]+t), cl(d[idx+1]+t), cl(d[idx+2]+t));
          d[idx] = c.r; d[idx+1] = c.g; d[idx+2] = c.b;
        }
      }
    } else {
      var kern = kernels[style] || kernels['Floyd-Steinberg'];
      var errR = new Float32Array(w*ht), errG = new Float32Array(w*ht), errB = new Float32Array(w*ht);
      for (var y = 0; y < ht; y++) {
        for (var x = 0; x < w; x++) {
          var idx = (y*w+x)*4;
          if (d[idx+3] === 0) continue;
          var ei = y*w+x;
          var r = cl(d[idx]+errR[ei]*difF), g = cl(d[idx+1]+errG[ei]*difF), b = cl(d[idx+2]+errB[ei]*difF);
          var c = near(r, g, b);
          var gf = golden ? (((x * phi + y * phi * phi) % 1) * 0.5 + 0.75) : 1;
          var er = (r-c.r)*divF*gf, eg = (g-c.g)*divF*gf, eb = (b-c.b)*divF*gf;
          d[idx] = c.r; d[idx+1] = c.g; d[idx+2] = c.b;
          for (var k = 0; k < kern.length; k++) {
            var nx = x+kern[k][0], ny = y+kern[k][1];
            if (nx >= 0 && nx < w && ny >= 0 && ny < ht) {
              var ni = ny*w+nx;
              errR[ni] += er*kern[k][2]; errG[ni] += eg*kern[k][2]; errB[ni] += eb*kern[k][2];
            }
          }
        }
      }
  ...
  function prevConvertToPal() {
    clearTimeout(_cpTimer);
    _cpTimer = setTimeout(function() {
      if (!_cpBk) return;
      var palRGB = [];
      for (var p = 0; p < userPalette.length; p++) {
        var hx = userPalette[p];
        palRGB.push({ r: parseInt(hx.slice(1,3),16), g: parseInt(hx.slice(3,5),16), b: parseInt(hx.slice(5,7),16) });
      }
      var style = document.getElementById('cp_style').value;
      var match = document.getElementById('cp_match').value;
      var divF = (+document.getElementById('cp_div').value) / 100;
      var difF = (+document.getElementById('cp_dif').value) / 100;
      var bri = +document.getElementById('cp_bri').value;
      var con = +document.getElementById('cp_con').value;
      var sat = +document.getElementById('cp_sat').value;
      var golden = document.getElementById('cp_gold').checked;
      var w = C.W, ht = C.H;
      var fi = An.cf;
      var fr = An.frames[fi];
      if (!fr || !_cpBk[fi]) return;
      for (var li = 0; li < fr.layers.length; li++) {
        if (!_cpBk[fi][li]) continue;
        var result = _cpRunLayer(_cpBk[fi][li], w, ht, palRGB, style, match, divF, difF, bri, con, sat, golden);
        var id = fr.layers[li].ctx.createImageData(w, ht);
        id.data.set(result);
        fr.layers[li].ctx.putImageData(id, 0, 0);
      }
      C.render();
    }, 50);
  }

  function apConvertToPal() {
    if (!_cpBk) { cMo(); return; }
    var palRGB = [];
    for (var p = 0; p < userPalette.length; p++) {
      var hx = userPalette[p];
    ...
    var golden = document.getElementById('cp_gold').checked;
    var w = C.W, ht = C.H;
    for (var fi = 0; fi < An.frames.length; fi++) {
      var fr = An.frames[fi];
      if (!_cpBk[fi]) continue;
      for (var li = 0; li < fr.layers.length; li++) {
        if (!_cpBk[fi][li]) continue;
        fr.layers[li].ctx.putImageData(_cpBk[fi][li], 0, 0);
      }
    }
    Hi.push();
    for (var fi = 0; fi < An.frames.length; fi++) {
      var fr = An.frames[fi];
      for (var li = 0; li < fr.layers.length; li++) {
        var src = fr.layers[li].ctx.getImageData(0, 0, w, ht);
        var result = _cpRunLayer(src, w, ht, palRGB, style, match, divF, difF, bri, con, sat, golden);
        var id = fr.layers[li].ctx.createImageData(w, ht);
        id.data.set(result);
        fr.layers[li].ctx.putImageData(id, 0, 0);
      }
    }
    _cpBk = null; cMo(); C.render(); An.uStrip(); Ly.ui();
  }
  ...
  function _cpCancel() {
    if (_cpBk) {
      for (var fi = 0; fi < An.frames.length; fi++) {
        ...
        var fr = An.frames[fi];
        for (var li = 0; li < fr.layers.length; li++) {
          if (_cpBk[fi][li]) fr.layers[li].ctx.putImageData(_cpBk[fi][li], 0, 0);
        }
      }
    }
    _cpBk = null; cMo(); C.render(); Ly.ui();
  }

  function mergeLayersDlg() {
    var f = An.frames[An.cf]; if (!f || f.layers.length < 2) { alert('Need at least 2 layers.'); return; }
    var h = '<h3>Merge Layers</h3>';
    h += '<label>Source (on top)<select id="ml_src" style="width:140px" onchange="var d=document.getElementById(\'ml_dst\');for(var i=0;i<d.options.length;i++)d.options[i].disabled=(d.options[i].value===this.value)">';
    for (var i = f.layers.length - 1; i >= 0; i--) h += '<option value="' + i + '">' + (i + 1) + ': ' + f.layers[i].name + '</option>';
    h += '</select></label>';
    h += '<label>Target (below)<select id="ml_dst" style="width:140px">';
    for (var i = f.layers.length - 1; i >= 0; i--) h += '<option value="' + i + '"' + (i === f.layers.length - 2 ? ' selected' : '') + '>' + (i + 1) + ': ' + f.layers[i].name + '</option>';
    h += '</select></label>';
    h += '<label>Apply to all frames<input type="checkbox" id="ml_all"></label>';
    h += '<p style="font-size:8px;color:var(--txd);margin-top:4px">Source layer is merged onto Target, then removed.</p>';
    h += '<div class="mb"><button class="bsc" onclick="U.cMo()">Cancel</button><button class="bp" onclick="U.apMergeLayers()">Merge</button></div>';
    sMo(h);
    var srcEl = document.getElementById('ml_src');
    var dstEl = document.getElementById('ml_dst');
    for (var i = 0; i < dstEl.options.length; i++) dstEl.options[i].disabled = (dstEl.options[i].value === srcEl.value);
  }

  function apMergeLayers() {
    var srcIdx = +document.getElementById('ml_src').value;
    var dstIdx = +document.getElementById('ml_dst').value;
    var allF = document.getElementById('ml_all').checked;
    cMo();
    if (srcIdx === dstIdx) { alert('Source and Target must be different.'); return; }
    Hi.push();
    var doFrame = function(fr) {
      if (srcIdx >= fr.layers.length || dstIdx >= fr.layers.length) return;
      var src = fr.layers[srcIdx], dst = fr.layers[dstIdx];
      if (!src || !dst) return;
      Ly.drawTo(dst.ctx, src);
      dst.ctx.globalAlpha = 1;
      dst.ctx.globalCompositeOperation = 'source-over';
      fr.layers.splice(srcIdx, 1);
      if (fr.activeLayer >= fr.layers.length) fr.activeLayer = fr.layers.length - 1;
    };
    if (allF) {
      for (var fi = 0; fi < An.frames.length; fi++) doFrame(An.frames[fi]);
    } else {
      doFrame(An.frames[An.cf]);
    }
    Ly.ui(); C.render(); An.uStrip();
  }
    ...
    document.addEventListener('keydown', e => {
      ...
      if (ctrl) {
        ...
        switch (e.key.toLowerCase()) {
          ...
          case "'": e.preventDefault(); C.togGrid(); break;
        }
        return;
      }
      ...
      if (shift) {
        ...
        if (k === 'n') { e.preventDefault(); Ly.add(); return; }
      }
      ...
      if (k === KEYS.goToFrame) {
        ...
        if (n) An.goTo(parseInt(n, 10) - 1);
        return;
      }
      if (k === KEYS.play) { An.pp(); return; }
      if (k === KEYS.onionSkin) { An.togOnion(); return; }
      if (k === KEYS.swapColors) { [C.pc, C.sc] = [C.sc, C.pc]; uCol(); return; }
  ...
  return {
    ...
    mergeLayersDlg, apMergeLayers,
    convertToPal, prevConvertToPal, apConvertToPal, _cpCancel, replaceColor,
    apRC: _lg(apRC),
    colorRange, apCR,
    gridCfg, applyGrid, checkerCfg, tiledCfg, textDlg, aTxt: _lg(aTxt),
    tweenDlg, apTw, togTheme, togCvOnly, togLeak, togColorMode, showKeys, apK, svKF, ldKF,
...
const IO = (() => {
  ...
  function save() {
    ...
    function _decodePCX(buf) {
      if (buf.length < 128 || buf[0] !== 0x0A) return null;
      var bpp = buf[3];
      var xMin = buf[4] | (buf[5] << 8), yMin = buf[6] | (buf[7] << 8);
      var xMax = buf[8] | (buf[9] << 8), yMax = buf[10] | (buf[11] << 8);
      var w = xMax - xMin + 1, h = yMax - yMin + 1;
      var planes = buf[65], bpl = buf[66] | (buf[67] << 8);
      if (w < 1 || h < 1 || w > 8192 || h > 8192 || bpl < 1) return null;
      var rgba = new Uint8ClampedArray(w * h * 4);
      var off = 128, scanBuf = new Uint8Array(bpl * planes);
      for (var y = 0; y < h; y++) {
        var si = 0;
        while (si < bpl * planes && off < buf.length) {
          var b = buf[off++];
          if ((b & 0xC0) === 0xC0) {
            var count = b & 0x3F, val = off < buf.length ? buf[off++] : 0;
            for (var j = 0; j < count && si < bpl * planes; j++) scanBuf[si++] = val;
          } else { scanBuf[si++] = b; }
        }
        if (planes === 3 && bpp === 8) {
          for (var x = 0; x < w; x++) { var pi = (y*w+x)*4; rgba[pi]=scanBuf[x]; rgba[pi+1]=scanBuf[bpl+x]; rgba[pi+2]=scanBuf[bpl*2+x]; rgba[pi+3]=255; }
        } else if (planes === 1 && bpp === 8) {
          for (var x = 0; x < w; x++) { var pi = (y*w+x)*4; rgba[pi]=scanBuf[x]; rgba[pi+1]=scanBuf[x]; rgba[pi+2]=scanBuf[x]; rgba[pi+3]=255; }
        } else if (planes === 1 && bpp === 1) {
          for (var x = 0; x < w; x++) { var pi = (y*w+x)*4; var v = ((scanBuf[x>>3]>>(7-(x&7)))&1)?255:0; rgba[pi]=v; rgba[pi+1]=v; rgba[pi+2]=v; rgba[pi+3]=255; }
        } else if (planes === 4 && bpp === 1) {
          for (var x = 0; x < w; x++) { var pi = (y*w+x)*4; var idx=0; for (var p=0;p<4;p++) idx|=(((scanBuf[p*bpl+(x>>3)]>>(7-(x&7)))&1)<<p); rgba[pi]=buf[16+idx*3]; rgba[pi+1]=buf[17+idx*3]; rgba[pi+2]=buf[18+idx*3]; rgba[pi+3]=255; }
        }
      }
      if (planes === 1 && bpp === 8 && off < buf.length) {
        while (off < buf.length && buf[off] !== 0x0C) off++;
        if (off < buf.length && buf[off] === 0x0C) {
          off++; var pal = [];
          for (var i = 0; i < 256 && off+2 <= buf.length; i++) { pal.push([buf[off],buf[off+1],buf[off+2]]); off+=3; }
          for (var y2=0; y2<h; y2++) for (var x2=0; x2<w; x2++) { var pi=(y2*w+x2)*4; var idx=rgba[pi]; if(idx<pal.length){rgba[pi]=pal[idx][0];rgba[pi+1]=pal[idx][1];rgba[pi+2]=pal[idx][2];} }
        }
      }
      return { w: w, h: h, rgba: rgba };
    }

    function _openPCX(file, asLayer) {
      var reader = new FileReader();
      reader.onload = function() {
        var buf = new Uint8Array(reader.result);
        var pcx = _decodePCX(buf);
        if (!pcx) { alert('Invalid or unsupported PCX file.'); return; }
        var cv = document.createElement('canvas');
        cv.width = pcx.w; cv.height = pcx.h;
        var cx = cv.getContext('2d');
        var id = cx.createImageData(pcx.w, pcx.h);
        id.data.set(pcx.rgba);
        cx.putImageData(id, 0, 0);
        var name = (file.name || 'PCX').replace(/\.[^.]+$/, '') || 'PCX';
        if (asLayer) {
          var frame = An.frames[An.cf]; if (!frame) return;
          if (T.floating) T.commitFloat();
          Hi.push();
          if (pcx.w > C.W || pcx.h > C.H) C.resize(Math.max(C.W, pcx.w), Math.max(C.H, pcx.h));
          var ly = Ly.mk(name);
          ly.ctx.drawImage(cv, Math.round((C.W - pcx.w) / 2), Math.round((C.H - pcx.h) / 2));
          An.frames[An.cf].layers.push(ly);
          An.frames[An.cf].activeLayer = An.frames[An.cf].layers.length - 1;
          Ly.ui(); C.render();
        } else {
          Hi.clear(); _dirty = false; T.resetStuck(); T.selection = null;
          C.init(pcx.w, pcx.h, 'transparent'); An.init();
          var layer = An.frames[0].layers[0];
          layer.name = name; layer.ctx.drawImage(cv, 0, 0);
          requestAnimationFrame(function() { C.zFit(); Ly.ui(); U.uCol(); An.uStrip(); });
        }
      };
      reader.onerror = function() { alert('Failed to read PCX file.'); };
      reader.readAsArrayBuffer(file);
    }

    function _encodePCX(w, h, rgba) {
      var bpl = w; if (bpl & 1) bpl++;
      var hdr = new Uint8Array(128);
      hdr[0] = 0x0A; hdr[1] = 5; hdr[2] = 1; hdr[3] = 8;
      var xMax = w-1, yMax = h-1;
      hdr[8] = xMax & 0xFF; hdr[9] = (xMax>>8) & 0xFF;
      hdr[10] = yMax & 0xFF; hdr[11] = (yMax>>8) & 0xFF;
      hdr[12] = 72; hdr[13] = 0; hdr[14] = 72; hdr[15] = 0;
      hdr[65] = 3;
      ...
      for (var y = 0; y < h; y++) {
        for (var plane = 0; plane < 3; plane++) {
          ...
          for (var x = 0; x < w; x++) line[x] = rgba[(y*w+x)*4 + plane];
          var x2 = 0;
          while (x2 < bpl) {
            var val = line[x2], run = 1;
            while (x2+run < bpl && run < 63 && line[x2+run] === val) run++;
            if (run > 1 || (val & 0xC0) === 0xC0) { body.push(0xC0|run); body.push(val); }
            else { body.push(val); }
            x2 += run;
          }
        }
      }
      var out = new Uint8Array(128 + body.length);
      out.set(hdr, 0);
      for (var i = 0; i < body.length; i++) out[128+i] = body[i];
      return out;
    }
    ...
    function _isSupportedImageFile(file) {
      ...
      return allowedTypes.has(file.type) || /\.(png|jpe?g|gif|webp|pcx|bmp)$/i.test(file.name || '');
    }

    function openImg() {
      const inp = document.createElement('input');
      inp.type = 'file';
      inp.accept = '.png,.jpg,.jpeg,.gif,.webp,.pcx,.bmp,image/png,image/jpeg,image/gif,image/webp,image/bmp';
      ...
      inp.onchange = () => {
        ...
        if (/\.pcx$/i.test(file.name || '')) { _openPCX(file, false); return; }
    ...
    function importImageAsLayer() {
      ...
      inp.onchange = () => {
        ...
        if (/\.pcx$/i.test(file.name || '')) { _openPCX(file, true); return; }
  ...
  function _encodeBMP(w, h, rgba) {
    var rowBytes = w * 3;
    var padBytes = (4 - (rowBytes % 4)) % 4;
    var rowStride = rowBytes + padBytes;
    var imgSize = rowStride * h;
    var fileSize = 54 + imgSize;
  ...
  function expBMP() {
    var fc = comp(An.frames[An.cf]);
    var cx = fc.getContext('2d');
    var id = cx.getImageData(0, 0, C.W, C.H);
    var bmp = _encodeBMP(C.W, C.H, id.data);
    var bl = new Blob([bmp], { type: 'image/bmp' });
    var url = BlobURLs.create(bl);
    var a = document.createElement('a');
    a.href = url; a.download = 'imagus.bmp'; a.click();
    setTimeout(function() { BlobURLs.revoke(url); }, 2000);
  }
  ...
  function expPCX() {
    ...
    var cx = fc.getContext('2d');
    var id = cx.getImageData(0, 0, C.W, C.H);
    var pcx = _encodePCX(C.W, C.H, id.data);
    var bl = new Blob([pcx], { type: 'application/octet-stream' });
    var url = BlobURLs.create(bl);
    var a = document.createElement('a');
    a.href = url; a.download = 'imagus.pcx'; a.click();
    setTimeout(function() { BlobURLs.revoke(url); }, 2000);
  }
  ...
  /** Export Grid Slices as ZIP */
  async function expGridSlices() {
    if (typeof JSZip === 'undefined') {
      alert('JSZip library not loaded. Add the JSZip script to Imagus.html.');
      return;
    }

    const gw = Math.max(1, C.gsz), gh = Math.max(1, C.gszH);
    const cols = Math.ceil(C.W / gw), rows = Math.ceil(C.H / gh);

    U.sMo(`<h3>Export Grid Slices</h3>
<p style="font-size:9px;color:var(--txd);margin-bottom:8px">
  Grid: ${gw}×${gh}px · ${cols} columns × ${rows} rows = ${cols * rows} cells
</p>
<label>Mode
  <select id="egs_mode">
    <option value="all">All cells</option>
    <option value="non-empty" selected>Non-empty only</option>
  </select>
</label>
<label>Source
  <select id="egs_src">
    <option value="composite" selected>Full composite</option>
    <option value="active-layer">Active layer only</option>
  </select>
</label>
<label>Detect duplicates
  <input type="checkbox" id="egs_dup" checked>
</label>
<div class="mb">
  <button class="bsc" onclick="U.cMo()">Cancel</button>
  <button class="bp" id="egs_go">Export ZIP</button>
</div>`);

    document.getElementById('egs_go').onclick = async () => {
      const mode = document.getElementById('egs_mode').value;
      const src = document.getElementById('egs_src').value;
      const detectDup = document.getElementById('egs_dup').checked;
      U.cMo();
      ...
      try {
        ...
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) {
            const sx = c * gw, sy = r * gh;
            const sw = Math.min(gw, w - sx), sh = Math.min(gh, h - sy);
            if (sw <= 0 || sh <= 0) continue;

            /* Extract tile pixels */
            const id = srcCtx.getImageData(sx, sy, sw, sh);
            const d = id.data;
            ...
            if (mode === 'non-empty' && empty) {
              slices.push({
                fileName: null,
                row: r, column: c, x: sx, y: sy,
                width: sw, height: sh, empty: true,
                duplicate: false, duplicateOf: null
              });
              continue;
            }

            const fileName = 'tile_r' + String(r).padStart(3, '0') +
                             '_c' + String(c).padStart(3, '0') + '.png';

            /* Duplicate detection via simple hash */
            ...
            if (detectDup && !empty) {
              ...
              const hashKey = sw + 'x' + sh + ':' + hash;
              if (hashMap.has(hashKey)) {
                isDup = true;
                dupOf = hashMap.get(hashKey);
              } else {
                hashMap.set(hashKey, fileName);
              }
            }

            /* Render tile to PNG blob */
            const tileCv = document.createElement('canvas');
            tileCv.width = sw; tileCv.height = sh;
            const tileCtx = tileCv.getContext('2d');
            tileCtx.putImageData(id, 0, 0);

            const blob = await new Promise(res => tileCv.toBlob(res, 'image/png'));
            if (blob) {
            ...
            slices.push({
              fileName,
              row: r, column: c, x: sx, y: sy,
              width: sw, height: sh, empty,
              duplicate: isDup, duplicateOf: dupOf
            });
          }
        }
        ...
        const url = BlobURLs.create(zipBlob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'imagus_grid_slices.zip';
        a.click();
        setTimeout(() => BlobURLs.revoke(url), 3000);
      } catch (err) {
        console.error('Grid slice export error:', err);
  ...
  return { exp, expGIF, expAnimWebP, expAPNG, expAVIF, expPCX, expBMP, expSpritesheet, expImageSeq, expGridSlices, importSeq, importImageAsLayer, save, openImg, loadProj, comp };
})();
...
const Ref = (() => {
  ...
  window.addEventListener('mousemove', e => {
    if (!dragging) return;
    const pw = panel.offsetWidth || 100;
    const ph = panel.offsetHeight || 40;
    const minL = -(pw - 40);
    const maxL = window.innerWidth - 40;
    const minT = 0;
    const maxT = window.innerHeight - 20;
    const nx = Math.max(minL, Math.min(maxL, e.clientX - dx));
    const ny = Math.max(minT, Math.min(maxT, e.clientY - dy));
...
const Prev = (() => {
  ...
  window.addEventListener('mousemove', function(e) {
    if (!dragging) return;
    var pw = panel.offsetWidth || 100;
    var minL = -(pw - 40);
    var maxL = window.innerWidth - 40;
    var maxT = window.innerHeight - 20;
    var nx = Math.max(minL, Math.min(maxL, e.clientX - ddx));
    var ny = Math.max(0, Math.min(maxT, e.clientY - ddy));
    panel.style.left = nx + 'px';
    panel.style.top = ny + 'px';
